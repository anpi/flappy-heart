...
public class Pipe extends Scrollable {
    ...
    public void update(float delta) {
        ...
        barDown.set(position.x, position.y + height + ORIGINAL_VERTICAL_GAP - (ORIGINAL_VERTICAL_GAP - VERTICAL_GAP)
                / 2, width, groundY
                - (position.y + height + ORIGINAL_VERTICAL_GAP - (ORIGINAL_VERTICAL_GAP - VERTICAL_GAP) / 2));
    ...
    public boolean collides(Bird bird) {
        if (position.x < bird.getX() + bird.getWidth()) {
            return (Intersector.overlaps(bird.getBoundingCircle(), barUp)
                    ...
                    || Intersector.overlaps(bird.getBoundingCircle(), skullUp) || Intersector.overlaps(
                    bird.getBoundingCircle(), skullDown));
        }
        return false;
    }
