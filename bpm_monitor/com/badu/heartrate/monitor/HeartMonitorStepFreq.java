...
public class HeartMonitorStepFreq implements HeartMonitor {
		...
		for (int f = 0; f < freq_range[1] - freq_range[0]; f++) {
			...
			for (int p = 0; p < phase_gran; p++) {
				...
				for (int t = 0; t < time_gran; t++) {
					double v = Math.sin((double) (f + freq_range[0])/freq_range[0] * t / time_gran * 2 * PI + (float) p / phase_gran * 2 * PI);
					if (Math.abs(v) > 0.5)
						trig_table[f][p][t] = 2;
					else
						trig_table[f][p][t] = -1;
	...
	public void addSample(byte[] data, int width, int height) {
		...
		int diff = (int) Math.abs( 1000 * (lastVal - imgAvg) / (nowTime - lastTime));
		lastVal = imgAvg;
		lastTime = nowTime;
